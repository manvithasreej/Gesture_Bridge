        ...
        if mode == 2:
            ...
            if results.multi_hand_landmarks is not None:
                ...
                for hand_landmarks, handedness in zip(
                    ...
                    # Only add if stable for some frames
                    if gesture_count == STABLE_FRAMES:
                        gesture_sentence.append(gesture)
            ...
            # Screen reflection #############################################################
            # Create text panel
            text_panel = np.ones((120, debug_image.shape[1], 3), dtype=np.uint8) * 255

            display_text = "".join(gesture_sentence[-40:])
            ...
            # Combine camera frame + text panel
            combined_frame = np.vstack((debug_image, text_panel))

            cv.imshow("Hand Gesture Recognition", combined_frame)
    print("\nFinal Sentence:")
    print("".join(gesture_sentence))
